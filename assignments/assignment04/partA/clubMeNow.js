...
function addClub(){
	...
	let clubAbrev=clubName.substring(0,2)
	let randomNum =Math.floor(Math.random() * (1599-199)) + 199;
	let newClub= [randomNum, clubAbrev,  clubName, 0, 0, 0, 0, 46.0, 100,  80];
...
function getAllClubs(){
    ...
    if(localStorage.getItem('clubs') === null){
        ...
        allClubs = JSON.parse(localStorage.getItem('clubs'))
    }
    return allClubs;
...
function undoLastShot() {
		//replace values with those on the undo club
		let undoClubsValues=JSON.parse(localStorage.getItem("clubsUndo"));

		//set new values as acquiredfrom undoClubs

		let undoClubsString = JSON.stringify(undoClubsValues);
		localStorage.setItem("clubs", undoClubsString );
	
		//reload to update UI display
